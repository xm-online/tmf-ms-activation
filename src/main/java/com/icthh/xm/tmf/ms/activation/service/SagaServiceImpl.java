...
public class SagaServiceImpl implements SagaService {
    ...
    public SagaTransaction createNewSaga(SagaTransaction sagaTransaction) {
        ...
        SagaTransaction saved = transactionUtils.withTransaction(() -> transactionRepository.saveAndFlush(sagaTransaction));
    ...
    private void doTask(SagaEvent sagaEvent, Context context) {
        ...
        try {
            ...
            if (TRUE.equals(taskSpec.getIsSuspendable()) && !continuation.isContinuationFlag()) {
                ...
                transactionUtils.withTransaction(() -> sagaEventRepository.saveAndFlush(sagaEvent.setStatus(SUSPENDED)));
            } else {
    ...
    private boolean isValidSpec(SagaEvent sagaEvent, DraftContext context) {
        if (!context.isValidSpec()) {
            ...
            transactionUtils.withTransaction(() -> sagaEventRepository.saveAndFlush(sagaEvent.setStatus(INVALID_SPECIFICATION)));
            return false;
        }
        return true;
    }
    ...
    private void continuation(SagaEvent sagaEvent, SagaTransaction transaction, SagaTransactionSpec transactionSpec,
        ...
        if (taskSpec.isIterable()) {
            finishIterableTask(sagaEvent, transaction, transactionSpec, taskSpec, taskContext);
        } else {
            finishTask(sagaEvent, transaction, transactionSpec, taskSpec, taskContext, sagaEvent.getIteration());
        }
    }
    ...
    private void finishIterableTask(SagaEvent sagaEvent, SagaTransaction transaction, SagaTransactionSpec transactionSpec,
                                    SagaTaskSpec taskSpec, Map<String, Object> taskContext) {
    ...
    private void resendEventsForDependentTasks(SagaTransactionSpec transactionSpec, SagaTaskSpec taskSpec, String transactionId) {
        if (TRUE.equals(transactionSpec.getCheckDependsEventually())) {
            ...
            dependentEvents.stream()
                ...
                .map(it -> transactionUtils.withTransaction(() -> sagaEventRepository.saveAndFlush(it)))
    ...
    private boolean isAllDependsTaskFinished(SagaEvent sagaEvent, Context context) {
        ...
        if (!notFinishedTasks.isEmpty()) {
            ...
            if (TRUE.equals(transactionSpec.getCheckDependsEventually())) {
                ...
                transactionUtils.withTransaction(() -> sagaEventRepository.saveAndFlush(sagaEvent));
            } else {
    ...
    public void cancelSagaTransaction(String sagaTxKey) {
        ...
        transactionUtils.withTransaction(() -> transactionRepository.saveAndFlush(sagaTransaction));
    }
    ...
    public void updateEventContext(String eventId, Map<String, Object> context) {
        sagaEventRepository.findById(eventId)
            ...
            .ifPresentOrElse(
                sagaEventRepository::saveAndFlush,
    ...
    private void writeLog(SagaEvent sagaEvent,
        ...
        if (logs.isEmpty()) {
            var sagaLogToSave = sagaLog;
            sagaLog = transactionUtils.withTransaction(() -> logRepository.saveAndFlush(sagaLogToSave));
